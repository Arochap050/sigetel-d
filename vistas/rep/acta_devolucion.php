...
$verificar = $connect->query("SELECT * FROM equipo_asig AS EA 
...
$connect->query("SELECT * FROM equipo_asig AS EA 
                    ...
                    WHERE status = 2 AND ID_Equipo_asig = $registro AND tipo_asg = 1; ");
} else {
...
$connect->query("SELECT * FROM equipo_asig AS EA 
                    ...
                    JOIN Marcas AS M ON M.id_Marca = DE.FK_Marca
                    JOIN Modelo AS MD ON MD.ID_Modelo = DE.FK_Modelo
                    JOIN Usuarios AS U ON U.ID_usuario = A.Responsable_ret
                    JOIN Estado_equipo AS ESS ON ESS.ID_Estado = A.status_eq_asg 
                    WHERE status = 2 AND ID_Equipo_asig = $registro AND tipo_asg = 1;");
}
...
$estadoEq = $dato["Estado"];
$n_control_d = $dato["n_control_d"];
$n_control_a = $dato["n_control_a"];

$fechaEntrega = strtotime($dato["fecha_devolucion"]);
...
class MYPDF extends TCPDF {
    ...
    public function Footer() {
        ...
        if ($this->getNumPages() == 1) { //primera pagina
            ...
            $this->Cell(86.5, 6, " Entregado por: $empleado" , 1, true, 'L', 0, '', 0, false, 'T', 'M');
            $this->Cell(1.4,5,'',0,0, 'L', 0, '',0);
            $this->Cell(85, 6, " Cargo: $cargo", 1, false, 'L', 0, '', 0, false, 'T', 'M');
            $this->Cell(86.5, 6, " Recibido por: $gerencia", 1, false, 'L', 0, '', 0, false, 'T', 'M');
...
$pdf->Cell(47, 5, "Nº Control: $n_control_d", 0, true, 'L', 0, '', 0, false, 'M', 'M');
...
$html = "<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;En la ciudad de Caracas, el día $fechaAsignacion, entre el Jefe de div. de Soporte y mesa de ayuda(E) de <b> TECNOLOGIA,</b> <b>$jefeSoporte</b>, titular de la cédula de identidad Nº $cedulaJefe, y el(la) trabajador(a)<b> $empleado</b>, titular de la cédula de identidad Nº $CedEmplaedo, de la <b>Div de $DivEmpleado.</b></p>";
...
$pdf->writeHTMLCell(175, 0, '', ''," <b>SEGUNDO:</b> Se reciben estos equipos, propiedad de Venezolana de Televisión C.A., (VTV), en concordancia con el acta de asignación de equipo <b>Nº Control: $n_control_a.</b>", '', 1, 0, true, 'J', true);
...
$pdf->Cell(1.4,5,'',0,0, 'L', 0, '',0);
$pdf->writeHTMLCell(171.5, 0, '', '',$dato["observacion_rec"], 'LRTB', 1, 0, true, 'J', true);
...
$pdf->writeHTMLCell(175, 40, '', '',(string) $dato["observacion_rec"], 'LR', 1, 0, true, 'J', true);
...
$responsablec = $dato["Responsable_ret"];
$r = $connect->query("SELECT p_Nombre_empleado, p_Apellido_empleado FROM Empleados AS E JOIN Usuarios AS U ON E.ID_empleado = U.FKEY_empleado WHERE ID_usuario = $responsablec");
...
$connect->query("SELECT * FROM Lineas_asignadas AS LA
            ...
            JOIN estado_linea AS e ON e.ID_Estado_linea = AL.estado_linea_asg
            ...
            WHERE status = 2 AND ID_Linea_asig = $registro AND tipo_asg = 1; ");
...
$pdf->SetFont('helvetica', '', 10);
$html = "<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;En la ciudad de Caracas, el día $fechaAsignacion, entre el Jefe de div. de Soporte y mesa de ayuda(E) de <b> TECNOLOGIA DE LA INFORMACIÓN Y COMUNICACIÓN</b> <b>$jefeSoporte</b>, titular de la cédula de identidad Nº $cedulaJefe, y el(la) trabajador(a)<b> $empleado</b>, titular de la cédula de identidad Nº $CedEmplaedo, de la <b>Div de $DivEmpleado.</b></p>";
